...
function scene:create(event)
  ...
  local windowInfo = composer.newText({
    string = composer.localized.get("CreateUser"),
  ...
  local function stopTimer()
    if createUserTimeout then
      timer.cancel(createUserTimeout)
      createUserTimeout = nil
    end
  end
  ...
  local function btnNextRelease()
    ...
    if not newName then
      ...
      composer.analytics.newEvent("design", {
        event_id = "createUser:attempt",
        area = composer.config.fullVersion
      })
      errorInfo.text = composer.localized.get("Loading")
      createUserTimeout = timer.performWithDelay(composer.config.serverTimeout, serverTimeout)
  ...
  nameTextField.x = usernameInfo.x + 120
  nameTextField.y = usernameInfo.y
