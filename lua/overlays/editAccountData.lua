...
function scene:create(event)
  ...
  local updateButton, changeState
  if isAndroid then
    textFieldSize = 30
  end
  local tempEmail = ""
  ...
  local info = composer.newText({
    string = composer.localized.get("AccountInfo"),
    size = 25,
    color = {
      1,
      1,
  ...
  emailFieldDescriptionText.y = 62
  ...
  passwordFieldDescriptionText.y = 98
  ...
  emailTextField.y = emailFieldDescriptionText.y
  ...
  passwordTextField.y = passwordFieldDescriptionText.y
  ...
  local function updateButtonEvent()
    ...
    if email and string.len(email) > 1 and password and string.len(password) > 1 then
      ...
      if not validPassword then
        ...
        composer.analytics.newEvent("design", {
          event_id = "updateAccount:emailAndPassword:invalidPassword",
          area = composer.config.fullVersion
        })
      elseif not validEmail then
        infoText.text = composer.localized.get(emailError)
        composer.analytics.newEvent("design", {
          event_id = "updateAccount:emailAndPassword:invalidEmail",
          area = composer.config.fullVersion
        })
      else
        infoText.text = composer.localized.get("SettingEmailAndPassword")
        composer.commHttps.changeEmail(validEmail, validPassword)
        tempEmail = validEmail
        stopTimer()
        timeoutTimer = timer.performWithDelay(composer.config.serverTimeout, timoutFromServer, 1)
        showUpdateButton(false)
        composer.analytics.newEvent("design", {
          event_id = "updateAccount:emailAndPassword:attempt",
          area = composer.config.fullVersion
        })
      end
      changeState = 3
    elseif email and string.len(email) > 1 then
      local validEmail, emailError = composer.validateInput.validateEmail(email)
      if not validEmail then
        ...
        composer.analytics.newEvent("design", {
          event_id = "updateAccount:onlyEmail",
          area = composer.config.fullVersion
        })
      end
      changeState = 1
    elseif password and string.len(password) > 1 then
      local validPassword, passwordError = composer.validateInput.validatePassword(password)
      if not validPassword then
        infoText.text = composer.localized.get(passwordError)
        composer.analytics.newEvent("design", {
  ...
  local function httpsCallback(data)
    if data.m == httpsFormat.changeEmail() then
      ...
      elseif changeState == 3 then
        infoText.text = composer.localized.get("AccountCreated")
        composer.data.playerInfo.email = tempEmail
        composer.analytics.newEvent("design", {
