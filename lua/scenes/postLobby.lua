...
function scene:create(event)
  ...
  function addChatBubble(playerId, chatId)
    ...
    chatButtonDropdown.x = UI_BASE_W * 0.26
    chatButtonDropdown.y = UI_BASE_H * 0.65
  ...
  local function setUpRankingTable(rankingTableFromComposer)
    ...
    local function updateAvatar(indexInList, username, pos)
        ...
        if includeAddFriendButton and composer.data.gameInfo.gameType ~= 0 then
            ...
            addFriendButtonList[pos].isVisible = false
            addFriendsButton.isVisible = true
            uiGroup:insert(addFriendButtonList[pos])
        end
    end
    ...
    local function setUpRankingTable(rankingTableFromComposer)
        ...
        if rankingTable and #rankingTable < 5 then
            ...
            uiGroup:insert(rankingTextGroup)
            effectGroup:toFront()
        else
            local errorText = composer.newText({
                string = composer.localized.get("ErrorNoPlayers"),
    ...
    local function updateDisplayGroups()
        uiGroup:insert(photoIcon)
        uiGroup:insert(backgroundTimeImage)
        uiGroup:insert(placeholderGroup)
        uiGroup:insert(mapName)
        uiGroup:insert(backgroundStatsImage)
        uiGroup:insert(returnToMenuButton)
        uiGroup:insert(rematchButton)
        uiGroup:insert(addFriendsButton)
        uiGroup:insert(friendButtonOverlay)
        uiGroup:insert(chatButton)
        uiGroup:insert(chatButtonOverlay)
        uiGroup:insert(chatButtonDropdown)
        uiGroup:insert(marketButton)
        uiGroup:insert(marketButtonOverlay)
        uiGroup:insert(effectGroup)
        if exitOnboarding then
            uiGroup:insert(exitOnboarding)
        end
    ...
    local function createChest()
        chest = basicBoostAdModule.init(composer.data.gameInfo.stats.fa, uiGroup, composer.gamesPlayed)
        if composer.data.gameInfo.stats.fa then
            local chestCoinTargetX = 95 + 355 - 14
            local chestCoinTargetY = UI_BASE_H - 44 + 39 - 14
    ...
    newStatsGroup.x = 95
    newStatsGroup.y = UI_BASE_H - 62
    uiGroup:insert(newStatsGroup)
    newTotalStatsGroup.x = 95
    newTotalStatsGroup.y = UI_BASE_H - 44
    uiGroup:insert(newTotalStatsGroup)
    ...
    uiGroup:insert(chatBubbleGroup)
    uiGroup:insert(chatButtonDropdown)
    addChatButtons()
    if composer.onboarding.isActive == true then
