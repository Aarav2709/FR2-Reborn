...
function Animation.new(name, timelines, duration)
  if not timelines then
    error("timelines cannot be nil", 2)
  end
  local self = {
    name = name,
    timelines = timelines,
...
local function binarySearch(values, target, step)
  ...
  while true do
    if target >= values[(current + 1) * step] then
      low = current + 1
    else
      high = current
    end
...
function Animation.EventTimeline.new()
  ...
  function self:apply(skeleton, lastTime, time, firedEvents, alpha)
    ...
    if time < lastTime then
      ...
      lastTime = -1
    elseif lastTime >= frames[frameCount - 1] then
      return
    end
    if time < frames[0] then
      return
    ...
    if lastTime < frames[0] then
      ...
      while true do
        if not (0 < frameIndex) or frames[frameIndex - 1] ~= frame then
          break
        end
    ...
    while frameCount > frameIndex and time >= frames[frameIndex] do
