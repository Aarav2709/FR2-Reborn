...
function SkeletonJson.new(attachmentLoader)
  ...
  function self:readSkeletonData(jsonText)
    ...
    for i, boneMap in ipairs(root.bones) do
      ...
      if parentName then
        ...
        if not parent then
          error("Parent bone not found: " .. parentName)
        end
      end
    ...
    if root.slots then
      for i, slotMap in ipairs(root.slots) do
        ...
        if color then
          slotData:setColor(tonumber(color:sub(1, 2), 16) / 255, tonumber(color:sub(3, 4), 16) / 255, tonumber(color:sub(5, 6), 16) / 255, tonumber(color:sub(7, 8), 16) / 255)
        end
  ...
  function readAttachment(name, map)
    ...
    elseif type == AttachmentType.mesh then
      ...
      if map.edges then
        mesh.edges = getArray(map, "edges", 1)
      end
        ...
        for timelineName, values in pairs(timelineMap) do
          if timelineName == "color" then
            ...
            for i, valueMap in ipairs(values) do
              ...
              timeline:setFrame(frameIndex, valueMap.time, tonumber(color:sub(1, 2), 16) / 255, tonumber(color:sub(3, 4), 16) / 255, tonumber(color:sub(5, 6), 16) / 255, tonumber(color:sub(7, 8), 16) / 255)
          ...
          elseif timelineName == "attachment" then
            ...
            for i, valueMap in ipairs(values) do
              ...
              attachmentName = attachmentName or nil
        ...
        for slotName, meshMap in pairs(slotMap) do
          ...
          for meshName, values in pairs(meshMap) do
            ...
            if not attachment then
              error("FFD attachment not found: " .. meshName)
            end
            timeline.slotIndex = slotIndex
            ...
            for i, valueMap in ipairs(values) do
              ...
              if not valueMap.vertices then
                ...
                elseif vertexCount > #verticesValue then
      ...
      for i, drawOrderMap in ipairs(drawOrderValues) do
        ...
        if offsets then
          ...
          for ii, offsetMap in ipairs(offsets) do
            ...
            if slotIndex == -1 then
              error("Slot not found: " .. offsetMap.slot)
            end
          ...
          while slotCount >= originalIndex do
            unchanged[unchangedIndex] = originalIndex
            unchangedIndex = unchangedIndex + 1
            originalIndex = originalIndex + 1
          end
    ...
    if events then
      ...
      for i, eventMap in ipairs(events) do
        ...
        if not eventData then
          error("Event not found: " .. eventMap.name)
        end
